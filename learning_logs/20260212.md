...
- 그래서 이 수업 이후 부분에서 `CASCADE` 로 설정을 바꿨을 때는 `DELTE FROM students WHERE student_id = 2;`가 아무런 문제 없이 가능합니다.
...
- AppUser 클래스를 정의하시오.
  ...
  3. SpringBoot 프로젝트를 실행시키고
    - id, name, username, password 순으로 1, 김일, kim1@test.com, kim123456 으로 row를 삽입하시오.
    - SELECT * FROM APP_USER;를 통해 row가 들어가있음을 확인하시오.
    - 캡처해서 md 파일에 옮겨놓고 자습하셔도 좋습니다.