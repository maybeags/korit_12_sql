...
2. TABLE을 생성하시오.
  - table 명 : products 
    - column 명 : 
      1. product_name varchar(50) / primary key
      2. stock int / not null

  - table 명 : orders
    - column 명 : 
        1. order_id / int / auto_increment / pk
        2. product_name varchar(50) / fk / restrict
        3. quantity int
```sql
...
# Transaction
## 정의
- 트랜잭션은 데이터베이스의 상태를 변화시키기 위해서 수행하는 _논리적인 작업 단위_ 를 의미함. 여러 개의 SQL문을 묶어서 모두 성공하거나, 아니면 아예 수해오디지 않은 상태로 처리하는 것을 의미함.
...
-- 
-- -- 재고 차감
 UPDATE products SET stock = stock - 2 WHERE product_name = 'iPhone18ProMax';
-- 
 COMMIT;
-- 
 SELECT * FROM products;
...
-- Table 'play_transaction.orders_miss' doesn't exist */
...
그렇다면 orders관련한 부분의 트랜잭션 내부에서 다시 트랜잭션을 재정의해야한다는 것을 의미합니다.

# Trigger
## 정의
- 테이블에 insert, update, delete와 같은 이벤트가 발생했을 때 데이터베이스가 자동으로 실행하도록 설정된 SQL 블록입니다.
- 활용 사례
...
- DB에 미리 정의해놓고 누군가가 orders에 대고 데이터를 집어넣었네(외부에서 나에게 주문을 넣었다면)? -> DB가 감시하고 있다가 products 테이블에 재고 빼는 거 실행함 -> 근데 음수값이야 -> '재고가 부족합니다'를 띄워야겠네. 와 같은 부분을 정의된 대로 실행시킵니다.
...
git add .
git commit -m "feat: vanilla sql lesson finished"
git push