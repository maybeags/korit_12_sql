...
- 예제 문제 : users에서 id 기준으로 오름차순 정렬해보기
```sql
...
- created_at 기준으로 내림 차순 정렬하시오.
```sql
SELECT * FROM users ORDER BY created_at DESC;
```

- ORDER BY는 컬럼명을 몰라도 정렬을 수행하는 것이 가능합니다. _몇 번 째 컬럼인지 숫자로 입력해도 가능_ 하다는 점입니다.
...
```
- 만약에 id를 기준으로 오름차순 되기를 바랐다면 결과값이 생각한 대로 나오지 않았을겁니다. ORDER BY 절의 실행 순서를 파악해야 할 필요가 있는데, _SELECT 절의 실행 결과를 기준으로 정렬_ 하기 때문에 이상의 쿼리문을 기준으로 했을 때는 1 번 컬럼이 username이 되어, username varchar 기준으로 오름차순 정렬되었다고 해석할 수 있습니다.
...
- users에서 city, id 컬럼만 출력하고, 거주 도시 기준으로 내림차순, 회원 아이디 기준 오름차순 정렬하시오.
```sql
SELECT city, id FROM users ORDER BY city DESC, id ASC;
```

- 이상의 ORDER BY 부분을 해석하면, 먼저 city 기준으로 내림차순 정렬한 뒤, 동일한 city 값 내에서는 id를 기준으로 오름차순 정렬하라는 의미가 됩니다. 그렇다면 ORDER BY 이후에 명시하는 컬럼명의 순서도 신경쓸 필요가 있다는 것을 확인할 수 있습니다.
...
```
3. orderdetails 테이블에서 제품 아이디를 기준으로 내림 차순 정렬하고, 같은 제품 아이디 기준으로 판매 수량을 기준으로 오름 차순 정렬하여 모든 컬럼을 출력하시오.
```sql
SELECT * FROM orderdetails ORDER BY product_id DESC, quantity ASC;
```
### SELECT / FROM / WHERE / ORDER BY 수준에성의 실무 예시
...
- 엑셀에서 합 / 평균 / 개수 세는 등의 작업을 할 때 함수를 사용해본 적이 있으실겁니다. SQL에서도 마찬가지로 함수를 사용해 동일한 작업을 할 수 있습니다.
  ...
  - 문자열을 잘라내거나 / 대소문자 변경 등을 수행하는 _일반 함수_ 
등이 존재합니다.

- 그룹별로 _집계 함수를 적용하기 위해 데이터를 나누는_ GROUP BY
  - 전체 데이터에 함수를 적용하기도 하지만(이하와 같이), 그룹별로 수치를 도출하는 일도 자주 있습니다. 그런 경우 GROUP BY를 이용해 데이터를 그룹으로 묶은 후 필요한 함수를 적용하면 됩니다.
```sql
...
- GROUP BY로 계산한 결과를 필터링하는 HAVING
  - GROUP BY를 사용하면 국가별 회원수를 추출하는 것은 가능한데, 그 중에서 마케팅 효율을 높이기 위해 국가별 회원 수 추출하고, 그 중에서 회원 수가 10 명 이상인 국가만 재추출하는 등 _집계 함수로 계산한 결과 중에서 조건에 맞는 데이터만 필터링_ 하는 경우도 존재합니다.
  - 그러면 일단 집계 함수가 적용된 이후에 WHERE을 써야 한다는건데, 집계되기 전에 미리 생각하고 WHERE절을 설계하는 것이 쉽지 않기 때문에 _GROUP BY 집계 함수의 `결과값`을 필터링할 때는 `HAVAING`_
...
```
  - LENGTH() : len()와 같네요.
    문자열의 길이를 return하는 것이 동일합니다. 그런데 DB 종류에 따라 LEN()으로 쓰는 것도 있습니다.
...
;
```
  - 이상의 경우에서 SUBSTR()을 적용한게 좀 더 모범답안에 해당합니다. 그 이유는
...
```sql
SELECT COUNT(DISTINCT id) AS 유저수 FROM users WHERE LENGTH(username) < 18;
```
## GROUP BY
이상까지의 수업 내용에서 SELECT 문에 집계 함수를 적용하고, 일반 함수를 SELECT와 WHERE절에서 사용해봤습니다. 집계함수를 SELECT 절로 쓰게 될 경우에 생겨나는 한계가 존재합니다. 데이터 전체가 아니라 원하는 기준으로 그룹을 나눠서 계산하기가 어렵다든지, 전체 회원 수가 아니라 국가별 회원 수를 나눈다든지 할 때가 되겠네요
...
- 문제3 : 국가별로 마케팅 수신 동의를 한 회원 수와 동의하지 않은 회원 수를 국가별 오름차순, 회원 수별 내림차순으로 추출하시오(컬럼명 동의사용자).
```sql
SELECT country, is_marketing_agree AS 동의(1), COUNT(DISTINCT id) AS 동의사용자
  ...
  ; 
```
이상의 코드에서 여러분이 주목해야 할 점은 ORDER BY 부분에서 '동의사용자 DESC'부분입니다. 원래 is_marketing_agree라는 이름의 column이 있기 때문에 우리가 재가공한 이름(동의사용자)를 기준으로 내림차순 정렬을 할 수 있는가 여부가 중요하다고 할 수 있겠습니다.
...
git commit -m "feat: 20260206 functions / GROUP BY"
